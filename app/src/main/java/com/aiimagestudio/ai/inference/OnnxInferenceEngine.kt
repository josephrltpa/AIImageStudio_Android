...
class OnnxInferenceEngine @Inject constructor(
    ...
    suspend fun runTextEncoder(
        ...
        // The mobile-exported text encoder expects int32 token ids (not the
        // int64 that ONNX's default tokenizer export uses) — the tensor's
        // *shape* array is still long[], only the token-id data itself needs
        // to be int32, matching "expected: (tensor(int32))" from ORT.
        val inputTensor = OnnxTensor.createTensor(
            env, IntBuffer.wrap(tokenIds), longArrayOf(1, tokenIds.size.toLong())
        )
        try {
